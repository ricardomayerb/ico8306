...
# Puede usar los grafico de base o los de ggplot2 
boxplot(notas$solemne, col="red")
boxplot(notas$examen, col = "green")
...
vector_medias <- vector(mode="numeric", length = muestras)

for(i in 1:muestras){
  esta_muestra <- sample(poblacion, size = n)
  media_muestral <- mean(esta_muestra)
  vector_medias[i] <- media_muestral
}
mean(vector_medias)
sd(vector_medias)
hist(vector_medias, col = "red", freq = F)
...
#la desvicaón estándar ahora es menor a la del ejercio anterior
