...
misdatos %>% summarize(cuantil_velocidad_viento = quantile(velocidad_viento, 0.025))

# 2.2 la mediana (2 pto)

misdatos %>% summarize(mediana_velocidad_viento = median(velocidad_viento))

# 2.3 el cuantil 0.975 (2 pto)
misdatos %>% summarize(cuantil_1_velocidad_viento = quantile(velocidad_viento, 0.975))
...
# 3.1 la media (2 pto)
misdatos %>% summarize(media_temperatura = mean(temperatura))

# 3.2 la desviación estandar    (2 pto)
misdatos %>% summarize(desviacion_estandar_temperatura = sd(temperatura))

# 3.3 la cantidad de observaciones (n)   (2 pto)
misdatos %>% summarize(desviacion_estandar_temperatura = n())
...
misdatos %>% summarize(hist_temperatura = hist(temperatura))

misdatos %>% summarize(hist_punto_rocio = hist(punto_rocio))

misdatos %>% summarize(hist_humedad = hist(humedad))

misdatos %>% summarize(hist_velocidad_viento = hist(velocidad_viento))
