...
quantile(datos_ie$velocidad_viento, c(0.025))
...
quantile(datos_ie$velocidad_viento, c(0.975))
...
desviación_estandar <-c(sd(datos_ie$temperatura))

desviación_estandar/(2612)^1/2 # -0.5 puntos


# desviación_estandar/(2612^(1/2))
...
hist(datos_ie$temperatura)

hist(datos_ie$humedad)

hist(datos_ie$punto_rocio)

hist(datos_ie$velocidad_viento)
...
A <-c(pnorm(datos_ie>-1))

B <-c(pnorm(datos_ie<1.2))
