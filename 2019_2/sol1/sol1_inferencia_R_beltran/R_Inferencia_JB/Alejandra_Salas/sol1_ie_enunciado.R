...
quantile(x, 0.025) # el cuantil 0.025 es 0.
# 2.2 la mediana (2 pto)
median(x) #La mediana es 9.20624
# 2.3 el cuantil 0.975 (2 pto)
quantile(x, 0.975) # el cuantil 0.975 es 23.0156
...
mean(y) #la media es 55.45093
# 3.2 la desviación estandar    (2 pto)
sd(y) # al desviacion estandar es 17.88357
# 3.3 la cantidad de observaciones (n)   (2 pto)
length(y) # son 2612 observaciones.
...
hist(a, main= "Temperatura")
b<- (misdatos$humedad)
hist(b, main= "Humedad")
c<- (misdatos$punto_rocio)
hist(c, main= "Punto de Rocio")
d<- (misdatos$velocidad_viento)
hist(d, main= "Velocidad del viento")
...
j<- pnorm(-1)
j
h-j # la probabilidad de que la variable este entre P(-1 <= z <= 1.2)
#es de 0.7262751.
